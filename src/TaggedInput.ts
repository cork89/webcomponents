export class TagInput extends HTMLElement {
    currentTags: HTMLElement[] = [];
    TAG_CHAR_LIMIT: number = 20;
    MAX_TAGS: number = 5;

    tagInput!: HTMLInputElement;
    tagFormInput!: HTMLInputElement;
    ...
    connectedCallback(): void {
        ...
        const tagFormInputElement =
            this.shadowRoot.getElementById("tag-form-input");
        if (!(tagFormInputElement instanceof HTMLInputElement)) {
            throw new Error(
                "tag-form-input element is missing or not an input"
            );
        }
        this.tagFormInput = tagFormInputElement;

        const tagContainerElement =
            this.shadowRoot.querySelector(".tag-container");
        if (!(tagContainerElement instanceof HTMLElement)) {
            throw new Error(
                "tag-container element is missing or not an HTMLElement"
            );
        }
    ...
    onKeyDown(event: KeyboardEvent): void {
        if (event.key === "," || event.key === "Enter") {
            ...
            event.preventDefault();
            const mostRecentTag =
                this.currentTags[this.currentTags.length - 1];
            if (mostRecentTag) {
